-- ============================================================
-- 019 — Persist the UTR / reference for fine payments
-- ------------------------------------------------------------
-- Until now a fine payment recorded only fine_paid_amount and
-- fine_paid_at on the EMI row. The transaction reference (UTR) the
-- retailer entered lived on payment_requests.utr and was never
-- propagated to the EMI, so per-EMI views (EMI schedule, fine
-- summary) could not show the UTR for a fine — especially for
-- fine-only payments, where the EMI's own `utr` column stays NULL
-- because no principal was collected.
--
-- This migration:
--   1. Adds emi_schedule.fine_utr (the reference for the fine payment).
--   2. Rewrites approve_payment_request so each fine UPDATE also
--      stamps fine_utr = COALESCE(fine_utr, v_request.utr).
--
-- The JS reconciler (lib/paymentReconcile.ts) writes the same column
-- for the approve-direct / admin-edit paths.
-- ============================================================

ALTER TABLE emi_schedule
  ADD COLUMN IF NOT EXISTS fine_utr TEXT;
...
-- Rewrite approve_payment_request to also persist fine_utr.
...
DO $$
BEGIN
  RAISE NOTICE '019: emi_schedule.fine_utr added; approve_payment_request now stamps fine_utr.';
END $$;
