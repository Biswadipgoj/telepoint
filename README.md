...
- [ ] Monthly collection CSV (pure CSV, no xlsx) — RUNNING customers only, grouped by retailer, chronologically sorted
- [ ] Running and Complete customer CSVs download separately
- [ ] EMI edit (admin): all status changes must happen INSIDE the Edit panel — no quick-action buttons on the outer row
- [ ] Payment modal: paying EMI 2 principal does NOT clear EMI 1 fine; both render as independent checkboxes
- [ ] Footer "Mastermind Behind The Code: Biswodip Goj" visible on every page
- [ ] Date math evaluates in Asia/Kolkata regardless of Vercel host timezone
